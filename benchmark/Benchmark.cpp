...
//    136.6 byte/iter
//   1285.5  KiB/sec
static void BM_Gobang_MCTS_Sequential(benchmark::State &state) {
    for (auto _ : state) {
...
BENCHMARK(BM_Gobang_MCTS_Sequential);

static void BM_Gobang_MCTS_Parallel(benchmark::State &state) {
    for (auto _ : state) {
        Server server(std::cin, std::cout);
        server.AddGame(R"({"type":"gobang","data":{}})"_json);
        server.AddState(R"({"gameID":1})"_json);
...
BENCHMARK(BM_Gobang_MCTS_Parallel);
