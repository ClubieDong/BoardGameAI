...
template <typename T>
static std::unique_ptr<ActionGenerator> CreateActionGenerator(const Game &game, const State &state, const nlohmann::json &data)
{
...
static const std::unordered_map<std::type_index, ActionGeneratorDataCreatorFunc> ActionGeneratorDataCreatorMap = {
    {typeid(tic_tac_toe::action_generator::Default), CreateActionGeneratorData<tic_tac_toe::action_generator::Default::Data>},
};

using ActionGeneartorCreatorFunc = std::unique_ptr<ActionGenerator> (*)(const Game &, const State &, const nlohmann::json &);
static const std::unordered_map<std::pair<std::type_index, std::string>, ActionGeneartorCreatorFunc> ActionGeneratorCreatorMap = {
...
std::unique_ptr<ActionGenerator> ActionGenerator::Create(const std::string &type, const Game &game, const State &state, const nlohmann::json &data)
{
