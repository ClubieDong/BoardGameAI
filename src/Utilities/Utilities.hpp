...
public:
    ...
    static std::pair<std::array<std::array<unsigned char, ColCount>, RowCount>, unsigned int> Json2Board(const nlohmann::json &data)
    {
    ...
    static bool NextEmptyGrid(const std::array<std::array<unsigned char, ColCount>, RowCount> &board, unsigned char &row, unsigned char &col)
    {
        do
        {
            if (++col >= ColCount)
